...
wsl bash -c "cd ~/battle-cats-rolls && env \$(cat .env | tr -d '\r' | xargs) ruby bin/build.rb tw"
...
start wsl bash -c "cd ~/battle-cats-rolls && bundle exec rackup -p 8080 -o 0.0.0.0"
